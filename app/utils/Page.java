...
public class Page  implements Serializable{
	 ...
	 * 确定跳转的方法
	 */
	public String submitMethod = "jumpPage";
	
	/**
	 * 输入页码框的ID
	 */
	public String pageInputId = "goPage";
	...
	public String getThemeNumberScript(){
		...
		case 5:
			return getFivetageThemeScript();
		default:
			return getDefaultThemeScript();
		}
	}

	private String getFivetageThemeScript() {
		...
		if (totalPageCount == 1) {
			strBuff.append(
					"<span class=\"xf_wylc_page_first5\"><span class=\"xf_wylc_page_firsthot5\">" + 1 + "</div>");
		...
		// strBuff.append("<span class=\"xf_wylc_page_first\">共计<span
		// class=\"xf_wylc_page_firsthot\">"+totalCount+"</span>"+pageTitle+"（共"+totalPageCount+"页）</span>");
		if (currPage != 1) {
			strBuff.append("<span class=\"xf_wylc_page_prev5\" onclick=\"javascript:" + defaultMethod + "(" + up + ","
					+ pageSize + ");\">上一页</span>");
		}
		...
		if (showPageCount != 2) {
			...
			for (i = inter[0]; i <= totalPageCount; i++) {
				...
				if (sum == showPageCount) {
					if (totalPageCount > i) {
						strBuff.append("&nbsp;<span class=\"xf_wylc_page_rele5\">...</span>");
					}
					strBuff.append("&nbsp;<span class=\"xf_wylc_page_rele5\" onclick=\"javascript:" + defaultMethod
							+ "(" + totalPageCount + "," + pageSize + ")\">" + totalPageCount + "</span>");
					break;
				}
				sum++;
				if (currPage == i) {
					strBuff.append("&nbsp;<span class=\"xf_wylc_page_ishot5\" onclick=\"javascript:" + defaultMethod
							+ "(" + i + "," + pageSize + ")\">" + i + "</span>");
				} else {
					strBuff.append("&nbsp;<span class=\"xf_wylc_page_rele5\" onclick=\"javascript:" + defaultMethod
							+ "(" + i + "," + pageSize + ")\">" + i + "</span>");
		...
		if (currPage != totalPageCount) {
			strBuff.append("&nbsp;<span class=\"xf_wylc_page_next5\" onclick=\"javascript:" + defaultMethod + "(" + down
					+ "," + pageSize + ")\">下一页</span>");
		}

		strBuff.append("<span class=\"xf_wylc_page_text\"></span><span class=\"xf_wylc_page_input5\">");
		strBuff.append("<input type=\"text\" id=\"" + pageInputId + "\" value=\"" + currPage + "\"></span>");
		strBuff.append(
				"<span class=\"xf_wylc_page_text\"></span><span class=\"xf_wylc_page_last5\" id=\"jumpPage\" onclick=\"javascript:"
						+ submitMethod + "(" + pageSize + ")\">跳转</span></div>");
		String script = "<script type=\"text/javascript\"> function " + submitMethod + "(pageSize){"
				+ "var currPage = $(\"#" + pageInputId + "\").val();" + "if(isNaN(currPage) || currPage == 0){"
				+ "alert(\"输入格式不正确，请重新输入!\");" + "return;" + "}" + "if(currPage > " + this.totalPageCount + "){"
				+ "alert(\"输入页数大于当前最大页!\");" + "$(\"#" + pageInputId + "\").val(" + this.totalPageCount + ");"
				+ "var currPage = " + this.totalPageCount + "}" + defaultMethod + "(currPage,pageSize);" + "}</script>";
	...
	private String getMixedThemeScript(){
		...
		strBuff.append("<span class=\"xf_wylc_page_text\">到第</span><span class=\"xf_wylc_page_input\">");
		strBuff.append("<input type=\"text\" id=\"" +pageInputId + "\" value=\""+currPage+"\"></span>");
		strBuff.append("<span class=\"xf_wylc_page_text\">页</span><span class=\"xf_wylc_page_last\" id=\"jumpPage\" onclick=\"javascript:" + submitMethod +"("+pageSize+")\">确定</span></div>");
		String script = "<script type=\"text/javascript\"> function "+submitMethod+"(pageSize){"
				+ "var currPage = $(\"#" +pageInputId + "\").val();"
				...
				+ "alert(\"输入页数大于当前最大页!\");"
				+ "$(\"#" +pageInputId + "\").val("+this.totalPageCount+");"
